...
export default function DcDetails({ feature }) {
  const { view } = useMapView();
  const { layers, layerView } = useLayers();
  // BUG: `setSelectedFeatures` is not part of SelectionContext and never has
  // been, so it is undefined here and the call below throws. Left as-is
  // because the intended behaviour isn't recoverable from the code -- see
  // CODEBASE_GUIDE.md, "Known issues".
  const { pushSelection, popupFeature, setSelectedFeatures, highlightHandleRef } = useSelection();
