/**
 * The attribute table below the map.
 *
 * Renders one tab per visible entry in FeatureTableDataContext; the rows come
 * from whoever wrote that entry (a filter widget, the selection tool, or a
...
export default function FeatureTable() {
  ...
  // Memoised because effects below key off it: `|| []` would otherwise hand
  // out a fresh array every render.
  const features = useMemo(() => currentTabData?.features || [], [currentTabData]);
 ...
 const handleRefresh = async () => {
  ...
  // Clicking a tab frames that tab's data on the map, so switching tabs
  // takes you to the rows you're looking at. Rows without geometry
  // (aggregated zone/analytics rows) are skipped; if a tab has none, the
  // map is left where it is.
  const zoomToTab = (tabId) => {
    if (!view) return;

    const targets = (tableData?.[tabId]?.features || []).filter((f) => f?.geometry);
    if (targets.length === 0) return;

    view.goTo({ target: targets, padding: [50, 50, 50, 50] }).catch((err) => {
      if (err.name !== "AbortError" && err.name !== "view:goto-interrupted") {
        console.error("Tab zoom failed:", err);
      }
    });
  };
      ...
      <div className="flex bg-[#1c1c1c] border-b border-gray-700 overflow-x-auto min-h-[32px]">
        {availableTabs.map((tabId) => (
          <button
            ...
            onClick={() => { setActiveTabId(tabId); zoomToTab(tabId); }}
