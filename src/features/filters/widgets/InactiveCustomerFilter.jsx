...
export default function InactiveCustomerFilter() {
    ...
    const fetchFilterData = async () => {
      ...
      try {
        ...
        data.features.forEach((feature) => {
          ...
          if (props.alarmstate !== null && props.alarmstate !== undefined) uniqueAlarms.add(props.alarmstate);
        });
        
        setStatuses(Array.from(uniqueStatuses));
        setAlarmStates(Array.from(uniqueAlarms).sort((a, b) => a - b));
      } catch (err) { setError("Failed to load filter options."); } finally { setIsLoading(false); }
    };
    fetchFilterData();
  }, []); 
  ...
  const handleActionClick = async () => {
    if (isCached) { setTableVisibility("inactive", true); return; }
    if (!view || !view.map) return;
    setIsFiltering(true); setError(null);
    ...
    try {
      ...
      targetLayer = new GeoJSONLayer({
        url: filteredUrl, title: "Customers_inactive",
        renderer: {
          type: "unique-value", field: "alarmstate", defaultSymbol: { type: "simple-marker", color: "gray", size: "8px", outline: null },
          uniqueValueInfos: alarmStates.map((alarm) => ({
            value: alarm, label: ALARM_LABELS[alarm] || `Unknown (${alarm})`,
            symbol: { type: "simple-marker", color: ALARM_COLORS[alarm] || "#999999", size: "10px", outline: null }
          }))
        },
        featureReduction: {
          type: "cluster", clusterRadius: "80px", clusterMinSize: "20px", clusterMaxSize: "40px",
          labelingInfo: [{ labelPlacement: "center-center", labelExpressionInfo: { expression: "$feature.cluster_count" }, symbol: { type: "text", color: "white", haloColor: "black", haloSize: "1px", font: { weight: "bold", size: "12px" } } }]
        }
      });
      
      view.map.add(targetLayer);
      await view.whenLayerView(targetLayer);
      const query = targetLayer.createQuery(); query.where = "1=1"; query.outFields = ["*"]; query.returnGeometry = true;
      const featureSet = await targetLayer.queryFeatures(query);
      if (addTableData) { addTableData("inactive", "Inactive Customers", featureSet.features, customerColumns, currentParams); }
      setAppliedSummary([ selectedStatus, ...(selectedAlarms.length > 0 ? [selectedAlarms.map((a) => ALARM_LABELS[a] || `Alarm ${a}`).join(", ")] : []) ].filter(Boolean));
    } catch (error) { setError("Failed to apply filter."); } finally { setIsFiltering(false); }
  };

  const clearFilter = () => {
    setSelectedStatus(""); setSelectedAlarms([]); 
    if (view && view.map) {
      const targetLayer = view.map.layers.find(layer => layer.title === "Customers_inactive");
      if (targetLayer) view.map.remove(targetLayer);
    }
    if (removeTableData) removeTableData("inactive");
    setAppliedSummary(null);
  };

  usePublishFilter({ id: "inactive_customers", label: "Inactive Customers", summary: appliedSummary, onClear: clearFilter });
  const isApplyDisabled = isLoading || isFiltering || selectedStatus === "" || selectedAlarms.length === 0; 

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      {error && (
        <CalciteNotice scale="m" kind="danger" icon="exclamation-mark-triangle" open>
      ...
      <CalciteLabel scale="m">
        Customer Status
        <CalciteSelect scale="m" value={selectedStatus} onCalciteSelectChange={(e) => setSelectedStatus(e.target.value)} disabled={isLoading || isFiltering}>
      ...
      <CalciteLabel scale="m">
        ...
        >
          {alarmStates.map((alarm, index) => (
            <CalciteComboboxItem key={index} value={alarm.toString()} textLabel={ALARM_LABELS[alarm] || `Unknown (${alarm})`} selected={selectedAlarms.includes(alarm.toString())} />
          ))}
        </CalciteCombobox>
      </CalciteLabel>

      <div style={{ display: "flex", gap: "0.5rem", marginTop: "1rem" }}>
        <CalciteButton scale="m" appearance="solid" width="half" onClick={handleActionClick} loading={isFiltering} disabled={isApplyDisabled}>
          {isCached ? "View Table" : (isLoading ? "Loading Data..." : "Apply Filter")}
        </CalciteButton>
        <CalciteButton scale="m" appearance="outline" kind="danger" width="half" onClick={clearFilter} disabled={isFiltering && !tableData?.inactive}>
          Clear
        </CalciteButton>
      </div>
    </div>
