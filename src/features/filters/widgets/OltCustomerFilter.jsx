...
export default function OltCustomerFilter() {
  ...
  const handleActionClick = async () => {
    ...
    try {
      ...
      setIsFiltered(true); setAppliedSummary([ selectedOlt, filterScope === "CURRENT_VIEW" ? "current view" : "all customers", ]);
    } catch (err) { setError("Failed to apply filter to map."); } finally { setIsApplying(false); }
  };

  const handleClear = () => {
    setSelectedOlt(""); if (customerLayerView) customerLayerView.filter = null; 
    removeWFSLayer();
    if (removeTableData) removeTableData("olt");
    setIsFiltered(false); setAppliedSummary(null);
  };

  usePublishFilter({ id: "olt_customers", label: "OLT", summary: appliedSummary, onClear: handleClear });
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "flex", gap: "0.5rem", marginTop: "0.5rem" }}>
        <CalciteButton scale="m" appearance="solid" onClick={handleActionClick} loading={isApplying} disabled={!selectedOlt || isLoading} style={{ flex: 1 }}>
