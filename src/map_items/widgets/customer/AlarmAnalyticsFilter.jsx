...
const ANALYTIC_OPTIONS = [
  ...
  { 
    value: "summaryByPort", 
    label: "2. Hardware Failure Metrics (Port/OLT)",
    description: "Tracks failures at the hardware level (OLT, Frame, Slot, Port). Use this to identify physical equipment that may be failing or damaged."
  },
  { 
    value: "alarmsByDate", 
    label: "3. Network Daily Trends",
    description: "Shows the daily volume of network faults. Use this to visualize overall network stability and pinpoint days with widespread outages."
  }
];
...
export default function AlarmAnalyticsFilter() {
  ...
  const fetchAndRenderHeatmap = async (reportData) => {
    const affectedAliases = [...new Set(reportData.map(row => row.alias).filter(Boolean))];
    ...
    geojsonData.features.forEach(feature => {
      ...
      const match = reportData.find(row => row.alias === alias);
      const repeatCount = match ? (Number(match.total_occurrences) || Number(match.occurrences) || 1) : 1;
        
      feature.properties.analytics_count = repeatCount;
      if (repeatCount > highestCount) highestCount = repeatCount;
    });
    ...
    const wfsLayer = new GeoJSONLayer({
      ...
      renderer: {
        ...
        maxPixelIntensity: highestCount, 
        minPixelIntensity: 0,
        radius: 18 
      }
    });
  ...
  const handleActionClick = async () => {
    ...
    try {
      ...
      if (selectedReport !== "alarmsByDate") {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem" }}>
        <CalciteLabel>
          Time Window (Days)
          <CalciteInput type="number" value={String(days)} min={1} max={90} onCalciteInputInput={(e) => setDays(e.target.value)} disabled={isApplying} />
        </CalciteLabel>

        <CalciteLabel>
          Min Repeats (Count)
          <CalciteInput type="number" value={String(minRepeats)} min={1} max={50} onCalciteInputInput={(e) => setMinRepeats(e.target.value)} disabled={isApplying} />
        </CalciteLabel>
      </div>
