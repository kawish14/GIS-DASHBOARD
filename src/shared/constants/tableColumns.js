/**
 * Column definitions for the attribute table (features/featureTable).
 *
 * Whoever pushes rows into FeatureTableDataContext passes the matching set
 * from here, so the same data always renders with the same headers and
 * formatting wherever it was produced.
 */
// constants/columns.js

// Helper function for fault duration (if needed)
const calculateDuration = (faultTimeStr, alarmstate) => {
  if (!faultTimeStr || alarmstate === 0) return "-";
...
export const customerColumns = [
  ...
  { key: "fault_time", label: "Fault Time" },
  {
    ...
    render: (attr) => calculateDuration(attr.fault_time, attr.alarmstate),
  },
